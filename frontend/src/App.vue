    ...
    <v-app :theme="darkMode ? 'dark' : 'light'">
        ...
        <div class="sidebar-brand" :class="{ 'sidebar-brand--rail': rail }">
          ...
          <transition name="fade">
            <div v-if="!rail" class="brand-text">
              ...
              <div class="brand-subtitle">Мониторинг инвестиций</div>
            </div>
          </transition>
        </div>
      ...
      <v-app-bar flat color="white" elevation="0" class="app-topbar" border="b">
        ...
        <template #append>
          ...
          <v-menu v-if="!authStore.isAdmin" :close-on-content-click="false" location="bottom end">
            ...
            <v-card min-width="320" max-width="400" class="rounded-lg">
              <v-card-title class="text-subtitle-1 font-weight-bold bg-grey-lighten-4 pb-2">
                Уведомления
              </v-card-title>
              <v-divider></v-divider>
              <v-list v-if="notifications.length > 0" class="pa-0" lines="two">
                <v-list-item 
                  ...
                  :class="{'bg-blue-grey-lighten-5': !n.is_read}" class="border-b"
                >
                  <v-list-item-subtitle class="text-caption text-grey mt-1">
                    {{ new Date(n.created_at).toLocaleString('ru-RU') }}
                  </v-list-item-subtitle>
                  <div class="text-body-2 mt-1 mb-1" style="color: var(--text-primary); line-height: 1.4;">
                    {{ n.message }}
                  </div>
      ...
      <v-main class="main-content" :style="{ backgroundColor: darkMode ? '#121212' : '#F5F7FA' }">
        <div class="content-wrapper">
          ...
          <v-alert
            v-if="authStore.isAuthenticated && authStore.user && !authStore.user.is_email_verified"
            type="warning"
            variant="tonal"
            class="mb-6 stat-card bg-white"
            border="start"
            icon="mdi-email-alert-outline"
          >
            <div class="d-flex align-center justify-space-between w-100">
              <div>
                <strong>Требуется подтверждение почты.</strong> 
                Для полного доступа подтвердите адрес электронной почты.
              </div>
              <v-btn color="warning" variant="elevated" size="small" :loading="isRequestingCode" @click="handleRequestCode">
                Подтвердить Email
              </v-btn>
            </div>
          </v-alert>
          ...
          <v-dialog v-model="showVerifyModal" max-width="450" persistent>
            <v-card class="stat-card pa-4">
              ...
              <v-card-text class="text-center">
                <p class="mb-4 text-body-2" style="color: var(--text-secondary)">
                  Мы отправили 6-значный код на <strong>{{ authStore.user?.email }}</strong>.
                </p>
...
import api, { notificationsAPI, authAPI } from '@/services/api'
...
// --- ЛОГИКА УВЕДОМЛЕНИЙ (КОЛОКОЛЬЧИК) ---
...
/* ... весь остальной CSS-стиль из твоего App.vue остается без изменений ... */
...
.brand-icon { width: 40px; height: 40px; border-radius: 10px; background: linear-gradient(135deg, #2E7D32, #4CAF50); display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
.brand-title { color: white; font-size: 15px; font-weight: 700; letter-spacing: 0.5px; line-height: 1.2; }
.brand-subtitle { color: rgba(255, 255, 255, 0.45); font-size: 11px; font-weight: 400; margin-top: 1px; }
...
.stat-card { border-radius: 12px !important; border: 1px solid var(--border) !important; transition: box-shadow 0.2s ease, transform 0.2s ease; }
.stat-card:hover { box-shadow: 0 4px 24px rgba(0, 0, 0, 0.06) !important; transform: translateY(-1px); }
</style>